...
function showSellerSidebar(sellerData){
    ...
    const searchURL = /http(s)?(:)?(\/\/)?|(\/\/)?(www\.)?/gi;
    const replaceURLWith = '';
    const sWebFormatted = sWeb.replace(searchURL, replaceURLWith);
...
<ul class="list-group" id="outerdiv">
        <li class="list-group-item  list-group-item-light list-group-item-action">
            ...
            ${`&ensp;${sStreet} ${sBuildingNum}, ${sZip} ${sCity}`}
        </li>

        <a class="list-group-item  list-group-item-light list-group-item-action" href="tel:${sPhone}">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-telephone-fill" viewBox="0 0 16 16">
...
$(document).ready(function() {    
    if (sEmail != ''){
    $('<a id="maildiv" class="list-group-item  list-group-item-light list-group-item-action" href="mailto:' + email + '">'
     + '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-envelope-fill" viewBox="0 0 16 16">'
...
$(document).ready(function() {    
    if (sWeb != ''){
    $('<a class="list-group-item  list-group-item-light list-group-item-action" href="//'+sWeb+'" target="_blank">'
            ...
            sellerSideBarUI += `<div class="card my-3 cursor-pointer" onClick="goToProductDetailsPage(${productId})">
                                    <div class="rounded bg-light">
                                        <div class="d-flex flex-row justify-content-start p-2 mx-1">
                                            <img class="cst-image-cover" src="${productImage}">
                                                        ...
                                                        <div class="d-flex flex-wrap justify-content-around ml-auto">`;
                                                            ...
                                                            productFeatureArray.forEach(function(featureType){
                                                                ...
                                                                if (feature != null && feature.length > 0){
                                                                    ...
                                                                    const featureName = featureObj.feature_name ? featureObj.feature_name : '';
                                                                    sellerSideBarUI += `<img class="cst-image-cover cst-feature-images p-2" src="${featureImagePath}" alt="">`;
                                                                }
                                                            })
                                                        sellerSideBarUI += `</div>
                                                </div>
                                        ...
                                        <div class="d-inline-flex justify-content-start rounded-pill cst-bg-gray cursor-pointer mb-2 ml-2 mr-2 text-truncate" onclick="goToProductionPointDeatailsScreen(${ppID})">
                                            <div>
                                                <img class="cst-feature-images rounded-circle m-1" src="${ppImagePath}" alt="">
                                            </div>
                                            <div class="my-auto mr-4">
                                                <h5 class="my-auto text-secondary text-truncate">${ppName}</h5>
                                             </div>
